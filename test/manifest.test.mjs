...
test("all example manifests are valid", async () => {
  ...
  assert.ok(files.length >= 2, "expected at least T1 and T2 examples");
...
test("invalid fixtures are all rejected", async () => {
  ...
  assert.ok(files.length >= 4, "expected the invalid fixtures to be present");
  for (const f of files) {
...
test("T1 without backup is rejected", async () => {
  const validate = await makeValidator();
  const data = await loadYaml(path.join(fixturesDir, "invalid-t1-missing-backup.yaml"));
